...
namespace Bjuro
{
    public partial class FormMain : Form
    {
        ...
        private void _employmentAgency_DealingRemoved(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < listViewDealings.Items.Count; i++)
            {
                if ((Dealing)listViewDealings.Items[i].Tag == dealing)
                {
                    listViewDealings.Items.RemoveAt(i);
                    break;
                }
            }
        }
        ...
        private void _employmentAgency_DealingAdded(object sender, EventArgs e)
        {
            ...
            if (dealing != null)
            {
                ...
                listViewItem.SubItems.Add(dealing.JobSeeker.ToString());
        ...
        private void редактироватьToolStripMenuItemDealings_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (_formDealing.ShowDialog() == DialogResult.OK)
                {
                    dealing = _formDealing.Dealing;
                    var listViewItem = listViewDealings.SelectedItems[0];
                    listViewItem.Text = dealing.Employer.ToString();
                    listViewItem.SubItems[1].Text = dealing.JobSeeker.ToString();
                    listViewItem.SubItems[2].Text = dealing.Post.ToString();
                    listViewItem.SubItems[3].Text = dealing.Commission.ToString();
                }
            }
            catch (Exception)
            {
        ...
        private void listViewDealings_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Delete)
            {
                try
