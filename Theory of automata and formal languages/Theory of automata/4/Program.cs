...
namespace _3
{
    ...
    class Program
    {
        ...
        static Hashtable tableid = new Hashtable();
        static string StrID = "";
        ...
        static bool lookUp(string obj)
        {
            if (tableid.ContainsKey(obj)){
                return true;
            }
            return false;
        }
        ...
        static void insert(string obj, string data)
        {
            try
            {
                tableid.Add(obj, data);
            }
            catch (Exception)
            {
                tableid.Remove(obj);
        ...
        static void viewTableID()
        {
            Console.WriteLine("\n");
            foreach(var item in tableid.Keys)
            {
                Console.WriteLine(tableid[item]);
            }
            Console.WriteLine("\n");
        }
        ...
        static void defineLexemes(string expression)
        {
            ...
            for (int i = 0; i < expression.Length; i++){
                ...
                if (lexeme != ""){
                    ...
                    if (!lookUp(lexeme)){
                        string data = "key - " + lexeme + " value - пусто" + " type - " + determine;
                        insert(lexeme, data);
                    }
